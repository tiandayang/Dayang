...
extension UIButton {
    ...
    class func dyButton(title: String?, image: UIImage?, heImage:UIImage?, bgImage: UIImage?, heBgImage: UIImage?) -> UIButton {
        
        let button = UIButton(type: .custom)
        button.setTitle(title, for: .normal)
        button.setTitleColor(UIColor.white, for: .normal)
        button.titleLabel?.font = UIFont.dy_systemFontWithSize(size: 15)
    ...
    class func dyButton(title: String?, bgImage: UIImage?, heBgImage: UIImage?) -> UIButton {
        
        return dyButton(title: title, image: nil, heImage: nil, bgImage: bgImage, heBgImage: heBgImage)
    }
    
    class func dyButton(image: UIImage?, heImage:UIImage?) -> UIButton {
        
        return dyButton(title: nil, image: image, heImage: heImage, bgImage: nil, heBgImage: nil)
    }
...
extension UIButton {
    ...
    ///   - mainColor: 还没倒计时的颜色
    ///   - countColor: 倒计时中的颜色
    func dyTimerButton(timeLimit: Int, title: String, countDownTitle: String, mainColor: UIColor, countColor: UIColor) -> Void {
        //倒计时时间
...
extension UIButton {
    ...
    enum dyButtonImagePosition : Int {
        case left    //图片在左
        case top     //图片在上
        case right   //图片在右
        case bottom  //图片在下
    ...
    ///   - position: 位置
    ///   - offset: 偏移量
    func ajustImagePosition(position: dyButtonImagePosition, offset: CGFloat) -> Void {
        //获取按钮图片的宽高
